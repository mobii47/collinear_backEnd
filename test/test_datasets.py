...
def test_dataset_not_valid(mock_session_local):
    ...
    assert "response_data" in response.json()  # Replace "response_data" with the expected response data
...
def test_get_datasets(mock_list_datasets):
    mock_list_datasets.return_value = ["dataset1", "dataset2"]  # Assuming list_datasets returns some datasets
    response = client.get("/datasets/")
    assert response.status_code == 200
    assert response.json() == {"datasets": ["dataset1", "dataset2"]}

def test_get_datasets_limit(mock_list_datasets):
    mock_list_datasets.return_value = ["dataset1", "dataset2", "dataset3"]  # Assuming list_datasets returns some datasets
    response = client.get("/datasets-10/")
    assert response.status_code == 200
    assert response.json() == {"datasets": ["dataset1", "dataset2", "dataset3"][:10]}  # Assuming we limit to 10 datasets
